...
APP_BASE_URL = (getattr(Config, "APP_URL", None) 
...
def place_outbound_call(to_number: str, from_number: Optional[str] = None,
    ...
    if not _validate_e164(from_number):
        _structured_log("invalid_from_number", level="ERROR",
                      message=f"Invalid E.164 from_number: {from_number}", trace_id=trace_id)
        return {"ok": False, "error": "invalid_from_number", "trace_id": trace_id}
